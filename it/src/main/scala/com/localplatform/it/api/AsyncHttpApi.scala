...
import com.localplatform.api.http.PeersApiRoute.{ConnectReq, connectFormat}
import com.localplatform.api.http.alias.CreateAliasV1Request
import com.localplatform.api.http.assets._
import com.localplatform.api.http.leasing.{LeaseCancelV1Request, LeaseV1Request, SignedLeaseCancelV1Request, SignedLeaseV1Request}
import com.localplatform.api.http.{AddressApiRoute, DataRequest}
import com.localplatform.features.api.ActivationStatus
import com.localplatform.http.DebugApiRoute._
import com.localplatform.http.DebugMessage._
import com.localplatform.http.{DebugMessage, RollbackParams, api_key}
...
import com.localplatform.state.{DataEntry, Portfolio}
...
object AsyncHttpApi extends Assertions {
  ...
  implicit class NodeAsyncHttpApi(n: Node) extends Assertions with Matchers {
    ...
    def payment(sourceAddress: String, recipient: String, amount: Long, fee: Long): Future[Transaction] =
      postJson("/local/payment", PaymentRequest(amount, fee, sourceAddress, recipient)).as[Transaction]
    ...
    def debugStateAt(height: Long): Future[Map[String, Long]] = getWithApiKey(s"/debug/stateLocal/$height").as[Map[String, Long]]
