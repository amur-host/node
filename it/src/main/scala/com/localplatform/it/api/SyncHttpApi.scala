...
import com.localplatform.api.http.AddressApiRoute
import com.localplatform.api.http.assets.SignedIssueV1Request
import com.localplatform.http.DebugMessage
...
object SyncHttpApi extends Assertions {
  ...
  implicit class NodeExtSync(n: Node) extends Assertions with Matchers {
    ...
    import com.localplatform.it.api.AsyncHttpApi.{NodeAsyncHttpApi => async}
  ...
  implicit class NodesExtSync(nodes: Seq[Node]) {
    ...
    import com.localplatform.it.api.AsyncHttpApi.{NodesAsyncHttpApi => async}
    ...
    def rollback(height: Int, returnToUTX: Boolean = true): Unit = {
      Await.result(
        Future.traverse(nodes) { node =>
          com.localplatform.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).rollback(height, returnToUTX)
        },
        ConditionAwaitTime
      )
    }
    ...
    def waitForHeight(height: Int): Unit = {
      Await.result(
        Future.traverse(nodes) { node =>
          com.localplatform.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).waitForHeight(height)
        },
        ConditionAwaitTime
      )
    }
