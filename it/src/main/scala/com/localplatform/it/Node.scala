...
import com.localplatform.it.util.GlobalTimer
import com.localplatform.settings.LocalSettings
import com.localplatform.state.EitherExt2
import com.localplatform.utils.{Base58, LoggerFacade}
...
abstract class Node(config: Config) extends AutoCloseable {
  ...
  val settings: LocalSettings = LocalSettings.fromConfig(config)
...
object Node {
  implicit class NodeExt(val n: Node) extends AnyVal {
    ...
    def fee(txTypeId: Byte, asset: String = "LOCAL"): Long = n.settings.feesSettings.fees(txTypeId).find(_.asset == asset).get.fee
