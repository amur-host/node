...
class MatcherTestSuite extends FreeSpec with Matchers with BeforeAndAfterAll with CancelAfterFailure with NodesFromDocker with ReportingTestName {
  ...
  "Check cross ordering between Alice and Bob " - {
    ...
    val order1 = matcherNode.placeOrder(aliceNode, aliceWavesPair, OrderType.SELL, 2.amur * Order.PriceConstant, aliceSellAmount, 1: Byte, 2.minutes)
    ...
    "sell order could be placed correctly" - {
      ...
      "and should match with buy order" in {
        ...
        val order2 = matcherNode.placeOrder(bobNode, aliceWavesPair, OrderType.BUY, 2.amur * Order.PriceConstant, 200, orderVersion)
      ...
      "submitting sell orders should check availability of asset" in {
        ...
        val badOrder =
          matcherNode.prepareOrder(bobNode, aliceWavesPair, OrderType.SELL, (19.amur / 10.0 * Order.PriceConstant).toLong, 300, orderVersion)
        ...
        val order3 =
          matcherNode.placeOrder(bobNode, aliceWavesPair, OrderType.SELL, (19.amur / 10.0 * Order.PriceConstant).toLong, 150, orderVersion)
      ...
      "buy order should match on few price levels" in {
        ...
        val order4 =
          matcherNode.placeOrder(aliceNode, aliceWavesPair, OrderType.BUY, (21.amur / 10.0 * Order.PriceConstant).toLong, 350, orderVersion)
      ...
      "order could be canceled and resubmitted again" in {
        ...
        val order4 =
          matcherNode.placeOrder(aliceNode, aliceWavesPair, OrderType.SELL, 2.amur * Order.PriceConstant, 100, orderVersion)
        order4.status should be("OrderAccepted")
      ...
      "buy order should execute all open orders and put remaining in order book" in {
        ...
        val order5 = matcherNode.placeOrder(bobNode, aliceWavesPair, OrderType.BUY, 2.amur * Order.PriceConstant, 130, orderVersion)
      ...
      "should consider UTX pool when checking the balance" in {
        ...
        def bobOrder = matcherNode.prepareOrder(bobNode, bobWavesPair, OrderType.SELL, 1.amur * Order.PriceConstant, bobAssetQuantity, orderVersion)
        ...
        val order7 =
          matcherNode.placeOrder(aliceNode, bobWavesPair, OrderType.BUY, 1.amur * Order.PriceConstant, bobAssetQuantity, orderVersion)
