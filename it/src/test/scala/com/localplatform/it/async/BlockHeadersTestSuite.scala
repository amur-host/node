  ...
  private def txRequestsGen(n: Int, fee: Long): Future[Unit] = {
    ...
    def requests(n: Int): Future[Unit] =
      Future
        .sequence {
          (1 to n).map { _ =>
            notMiner.transfer(notMiner.address, firstAddress, (1 + Random.nextInt(10)).local, fee)
          }
        }
        .map(_ => ())
