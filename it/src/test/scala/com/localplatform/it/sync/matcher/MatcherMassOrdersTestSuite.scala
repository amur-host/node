...
class MatcherMassOrdersTestSuite
  ...
  "Create orders with statuses FILL, PARTIAL, CANCELLED, ACTIVE" - {
    ...
    val aliceSecondLocalPair = AssetPair(ByteStr.decodeBase58(aliceSecondAsset).toOption, None)
    ...
    val aliceOrderIdFill = matcherNode
      .placeOrder(aliceNode, aliceSecondLocalPair, OrderType.SELL, Order.PriceConstant, 3, orderVersion, 10.minutes)
      .message
      .id

    val alicePartialOrderId = matcherNode
      .placeOrder(aliceNode, aliceSecondLocalPair, OrderType.SELL, Order.PriceConstant, 3, orderVersion, 10.minute)
      .message
      .id

    val aliceOrderToCancelId =
      matcherNode
        .placeOrder(aliceNode, aliceSecondLocalPair, OrderType.SELL, Order.PriceConstant, 3, orderVersion, 70.second)
        .message
        .id

    val aliceActiveOrderId = matcherNode
      .placeOrder(aliceNode, aliceSecondLocalPair, OrderType.SELL, Order.PriceConstant + 1, 3, orderVersion, 10.minute)
      .message
      .id

    matcherNode.waitOrderStatus(aliceSecondLocalPair, aliceOrderToCancelId, "Cancelled", 2.minutes)

    //Bob orders should partially fill one Alice order and fill another
    ordersRequestsGen(2, bobNode, aliceSecondLocalPair, OrderType.BUY, 2)

    //check orders after filling
    matcherNode.waitOrderStatus(aliceSecondLocalPair, alicePartialOrderId, "PartiallyFilled")
    ...
    "Mass orders creation with random lifetime. Active orders still in list" in {
      ...
      ordersRequestsGen(orderLimit, aliceNode, aliceLocalPair, OrderType.SELL, 3)
      //wait for some orders cancelled
      Thread.sleep(100000)
      ordersRequestsGen(orderLimit, bobNode, aliceLocalPair, OrderType.BUY, 2)
      ...
      matcherNode.waitOrderStatus(aliceSecondLocalPair, aliceActiveOrderId, "Accepted")
      matcherNode.waitOrderStatus(aliceSecondLocalPair, alicePartialOrderId, "PartiallyFilled")
    }
