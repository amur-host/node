...
import com.localplatform.it.sync.{minFee, transferAmount}
...
import com.localplatform.transaction.Proofs
import com.localplatform.transaction.lease.{LeaseCancelTransactionV2, LeaseTransactionV2}
...
class LeaseSmartContractsTestSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("set contract, make leasing and cancel leasing") {
    ...
    notMiner.assertBalances(firstAddress,
                            balance1 + 10 * transferAmount - (2 * minFee + 0.2.local),
                            eff1 + 9 * transferAmount - (2 * minFee + 0.2.local))
    ...
    notMiner.assertBalances(firstAddress,
                            balance1 + 10 * transferAmount - (3 * minFee + 2 * 0.2.local),
                            eff1 + 10 * transferAmount - (3 * minFee + 2 * 0.2.local))
