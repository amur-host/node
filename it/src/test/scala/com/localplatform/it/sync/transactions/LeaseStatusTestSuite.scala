...
import com.localplatform.transaction.lease.LeaseTransaction.Status.{Active, Canceled}
...
object LeaseStatusTestSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""local {
       |   miner{
       |      enable = yes
  ...
  private val notMinerConfig = ConfigFactory.parseString(s"""local {
       |   miner.enable = no
