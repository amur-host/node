...
import com.localplatform.it.api.UnexpectedStatusCodeException
import com.localplatform.it.sync.{calcDataFee, minFee}
import com.localplatform.it.transactions.BaseTransactionSuite
import com.localplatform.it.util._
import com.localplatform.state.{BinaryDataEntry, BooleanDataEntry, ByteStr, DataEntry, EitherExt2, IntegerDataEntry, StringDataEntry}
import com.localplatform.utils.Base58
import org.scalatest.{Assertion, Assertions}
import play.api.libs.json._
import com.localplatform.api.http.SignedDataRequest
import com.localplatform.transaction.DataTransaction
...
class DataTransactionSuite extends BaseTransactionSuite {
  ...
  test("sender's local balance is decreased by fee.") {
  ...
  test("cannot transact without having enough local") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)

    val data = List(BooleanDataEntry("bool", false))
    assertBadRequestAndResponse(sender.putData(firstAddress, data, balance1 + 1), "negative local balance")
    nodes.waitForHeightArise()
    notMiner.assertBalances(firstAddress, balance1, eff1)

    val leaseAmount = 1.local
