...
import com.localplatform.it.TransferSending
...
import com.localplatform.account.AddressOrAlias
...
class TransferTransactionV1Suite extends BaseTransactionSuite with TransferSending with CancelAfterFailure {
  ...
  test("asset transfer changes sender's and recipient's asset balance; issuer's.local balance is decreased by fee") {
  ...
  test("local transfer changes local balances and eff.b.") {
    val (firstBalance, firstEffBalance)   = notMiner.accountBalances(firstAddress)
    val (secondBalance, secondEffBalance) = notMiner.accountBalances(secondAddress)
  ...
  test("invalid signed local transfer should not be in UTX or blockchain") {
  ...
  test("can not make transfer without having enough balance") {
    ...
    assertBadRequestAndResponse(sender.transfer(secondAddress, firstAddress, secondBalance + 1.local, minFee),
