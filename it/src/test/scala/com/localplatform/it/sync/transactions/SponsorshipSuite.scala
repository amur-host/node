...
import com.localplatform.api.http.assets.SignedSponsorFeeRequest
import com.localplatform.it.api.SyncHttpApi._
import com.localplatform.it.sync._
import com.localplatform.it.transactions.NodesFromDocker
import com.localplatform.it.util._
import com.localplatform.it.{NodeConfigs, ReportingTestName}
import com.localplatform.state.{ByteStr, Sponsorship}
import com.localplatform.transaction.assets.SponsorFeeTransaction
...
class SponsorshipSuite extends FreeSpec with NodesFromDocker with Matchers with ReportingTestName with CancelAfterFailure {
  ...
  override def nodeConfigs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw("local.blockchain.custom.functionality.blocks-for-feature-activation=1"))
      .overrideBase(_.raw("local.blockchain.custom.functionality.feature-check-blocks-period=1"))
      .withDefault(1)
  ...
  "Fee in sponsored asset works fine" - {
    ...
    val sponsorLocalBalance = sponsor.accountBalances(sponsor.address)._2
    val minerLocalBalance   = miner.accountBalances(miner.address)._2
    ...
    "check balance before test accounts balances" in {
      ...
      sponsor.assertBalances(sponsor.address, sponsorLocalBalance - 2 * issueFee - minFee)
    ...
    val minerLocalBalanceAfterFirstXferTest   = minerLocalBalance + 2.local + minFee + Sponsorship.FeeUnit * SmallFee / minSponsorFee
    val sponsorLocalBalanceAfterFirstXferTest = sponsorLocalBalance - 2.local - minFee - Sponsorship.FeeUnit * SmallFee / minSponsorFee
...
//        sponsorTx.head.count(tx => tx.sender.contains(sponsor.address) || tx.recipient.contains(sponsor.address)) shouldBe 4
      ...
      "sponsor should receive sponsored asset as fee, local should be written off" in {
        miner.assertAssetBalance(sponsor.address, sponsorAssetId, sponsorAssetTotal / 2 + SmallFee)
        miner.assertBalances(sponsor.address, sponsorLocalBalanceAfterFirstXferTest)
      }

      "miner local balance should be changed" in {
        miner.assertBalances(miner.address, minerLocalBalanceAfterFirstXferTest)
      }
    }
    ...
    "assets balance should contain sponsor fee info and sponsor balance" in {
      ...
      nodes.waitForHeightAriseAndTxPresent(sponsorLeaseSomeLocal)
    ...
    "local fee depends on sponsor fee and sponsored token decimals" in {
      val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.local, LargeFee, None, Some(sponsorAssetId)).id
      nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
      assert(!transferTxCustomFeeAlice.isEmpty)
      ...
      miner.assertBalances(
        sponsor.address,
        sponsorLocalBalanceAfterFirstXferTest - Sponsorship.FeeUnit * LargeFee / Token - leasingFee,
        sponsorLocalBalanceAfterFirstXferTest - Sponsorship.FeeUnit * LargeFee / Token - leasingFee - leasingAmount
      )
      miner.assertBalances(miner.address, minerLocalBalanceAfterFirstXferTest + Sponsorship.FeeUnit * LargeFee / Token + leasingFee)
    }
    ...
    "cancel sponsorship" - {
      ...
      "check sponsor and miner balances after cancel" in {
        miner.assertBalances(
          sponsor.address,
          sponsorLocalBalanceAfterFirstXferTest - Sponsorship.FeeUnit * LargeFee / Token - leasingFee - issueFee,
          sponsorLocalBalanceAfterFirstXferTest - Sponsorship.FeeUnit * LargeFee / Token - leasingFee - leasingAmount - issueFee
        )
        miner.assertBalances(miner.address, minerLocalBalanceAfterFirstXferTest + Sponsorship.FeeUnit * LargeFee / Token + leasingFee + issueFee)
      }
    ...
    "set sponsopship again" - {
      ...
      "make transfer with new min sponsored fee" in {
        ...
        val aliceLocalBalance   = alice.accountBalances(alice.address)
        val bobAssetBalance     = bob.assetBalance(bob.address, sponsorAssetId).balance
        val bobLocalBalance     = bob.accountBalances(bob.address)
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.local, TinyFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        nodes.waitForHeightArise()

        val localFee = Sponsorship.FeeUnit * TinyFee / TinyFee
        sponsor.assertBalances(sponsor.address, sponsoredBalance._1 - localFee, sponsoredBalance._2 - localFee)
        ...
        alice.assertBalances(alice.address, aliceLocalBalance._2 - 1.local)
        bob.assertBalances(bob.address, bobLocalBalance._1 + 1.local, bobLocalBalance._2 + 1.local)
        bob.assertAssetBalance(bob.address, sponsorAssetId, bobAssetBalance)
        miner.assertBalances(miner.address, minerBalance._2 + localFee)
      ...
      "make transfer with updated min sponsored fee" in {
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.local, LargeFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        val localFee = Sponsorship.FeeUnit * LargeFee / LargeFee
        nodes.waitForHeightArise()
    ...
    "issue asset make sponsor and burn and reissue" in {
      ...
      nodes.waitForHeightAriseAndTxPresent(aliceTransferLocal)
      nodes.waitForHeightArise()

      val totalLocalFee = Sponsorship.FeeUnit * SmallFee / Token + issueFee + sponsorFee + burnFee + minFee + issueFee
      miner.assertBalances(miner.address, minerBalance._1 + totalLocalFee)
      sponsor.assertBalances(sponsor.address, sponsorBalance._1 - totalLocalFee, sponsorBalance._2 - totalLocalFee)
    ...
    "miner is sponsor" in {
      ...
      nodes.waitForHeightAriseAndTxPresent(aliceSponsoredTransferLocal)
      nodes.waitForHeightArise()
