...
import com.localplatform.it.NodeConfigs.Default
...
import com.localplatform.state.{EitherExt2, Sponsorship}
...
class CustomFeeTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("make transfer with sponsored asset") {
    ...
    val sponsorship = Sponsorship.toLocal(transferFee, assetToken)
...
object CustomFeeTransactionSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""
      | local.fees.transfer.$assetId = 100000
      | local.blockchain.custom.functionality {
  ...
  private val notMinerConfig = ConfigFactory.parseString("local.miner.enable=no").withFallback(minerConfig)
