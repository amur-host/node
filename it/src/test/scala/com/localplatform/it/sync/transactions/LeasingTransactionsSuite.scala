...
class LeasingTransactionsSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("leasing local decreases lessor's eff.b. and increases lessee's eff.b.; lessor pays fee") {
  ...
  test("can not make leasing without having enough balance") {
    ...
    assertBadRequestAndResponse(sender.lease(secondAddress, firstAddress, balance2 + 1.local, minFee), "Reason: Cannot lease more than own")
  ...
  test("can not make leasing without having enough local for fee") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
  ...
  test("can not make leasing without having enough your local to self") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    assertBadRequestAndResponse(sender.lease(firstAddress, firstAddress, balance1 + 1.local, minFee), "Transaction to yourself")
    nodes.waitForHeightArise()
