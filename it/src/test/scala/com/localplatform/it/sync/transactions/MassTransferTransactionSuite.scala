...
import com.localplatform.account.Alias
import com.localplatform.api.http.assets.{MassTransferRequest, SignedMassTransferRequest}
import com.localplatform.transaction.transfer.MassTransferTransaction.{MaxTransferCount, Transfer}
import com.localplatform.transaction.transfer.TransferTransaction.MaxAttachmentSize
...
class MassTransferTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("asset mass transfer changes asset balances and sender's.local balance is decreased by fee.") {
  ...
  test("local mass transfer changes local balances") {
  ...
  test("can not make mass transfer without having enough local") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
    val transfers        = List(Transfer(secondAddress, balance1 / 2), Transfer(thirdAddress, balance1 / 2))

    assertBadRequestAndResponse(sender.massTransfer(firstAddress, transfers, calcMassTransferFee(transfers.size)), "negative local balance")
  ...
  test("reporting MassTransfer transactions") {
    ...
    val transfers = List(Transfer(firstAddress, 5.local), Transfer(secondAddress, 2.local), Transfer(thirdAddress, 3.local))
    ...
    assert((txSender \ "totalAmount").as[Long] == 10.local)
    ...
    assert((txRecipient \ "totalAmount").as[Long] == 10.local)
  ...
  test("reporting MassTransfer transactions to aliases") {
    ...
    val transfers = aliases.map { alias =>
      Transfer(Alias.buildWithCurrentNetworkByte(alias).explicitGet().stringRepr, 2.local)
    }
    val txId = sender.massTransfer(firstAddress, transfers, 300000).id
    nodes.waitForHeightAriseAndTxPresent(txId)
