...
import com.localplatform.metrics.{Instrumented, TxsInBlockchainStats}
import com.localplatform.mining.{MiningConstraint, MiningConstraints, MultiDimensionalMiningConstraint}
import com.localplatform.settings.LocalSettings
import com.localplatform.state.diffs.BlockDiffer
import com.localplatform.state.reader.{CompositeBlockchain, LeaseDetails}
...
import com.localplatform.block.{Block, BlockHeader, MicroBlock}
import com.localplatform.transaction.Transaction.Type
import com.localplatform.transaction.ValidationError.{BlockAppendError, GenericError, MicroBlockAppendError}
...
class BlockchainUpdaterImpl(blockchain: Blockchain, settings: LocalSettings, time: Time)
    extends BlockchainUpdater
    with NG
    with ScorexLogging
    with Instrumented {

  import com.localplatform.state.BlockchainUpdaterImpl._
  ...
  /** Retrieves Local balance snapshot in the [from, to] range (inclusive) */
  ...
  override def localDistribution(height: Int): Map[Address, Long] = ngState.fold(blockchain.localDistribution(height)) { ng =>
    val innerDistribution = blockchain.localDistribution(height)
  ...
  /** Builds a new portfolio map by applying a partial function to all portfolios on which the function is defined.
    *
    * @note Portfolios passed to `pf` only contain Local and Leasing balances to improve performance */
