...
import com.localplatform.transaction.ValidationError.{GenericError, OrderValidationError}
...
object ExchangeTransactionDiff {
  ...
  def apply(blockchain: Blockchain, height: Int)(tx: ExchangeTransaction): Either[ValidationError, Diff] = {
    ...
    for {
      ...
      def localPortfolio(amt: Long) = Portfolio(amt, LeaseBalance.empty, Map.empty)

      val feeDiff = Monoid.combineAll(
        Seq(
          Map(matcher -> localPortfolio(t.buyMatcherFee + t.sellMatcherFee - t.fee)),
          Map(buyer   -> localPortfolio(-t.buyMatcherFee)),
          Map(seller  -> localPortfolio(-t.sellMatcherFee))
        ))
