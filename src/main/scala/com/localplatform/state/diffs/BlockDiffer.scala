...
import com.localplatform.state.patch.{CancelAllLeases, CancelInvalidLeaseIn, CancelLeaseOverflow}
...
import com.localplatform.transaction.ValidationError.ActivationError
  ...
  private def clearSponsorship(blockchain: Blockchain, portfolio: Portfolio, height: Int, fs: FunctionalitySettings): Portfolio = {
    if (height >= Sponsorship.sponsoredFeesSwitchHeight(blockchain, fs)) {
      ...
      val sponsoredLocal = sponsoredAssets.map {
        case (_, totalFee, baseFee) => Sponsorship.toLocal(totalFee, baseFee)
      }.sum
      unsponsoredPf.copy(balance = unsponsoredPf.balance + sponsoredLocal)
    } else portfolio
  }
