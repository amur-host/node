package com.localplatform.state.reader
...
import com.localplatform.transaction.{AssetId, Transaction, ValidationError}
...
class CompositeBlockchain(inner: Blockchain, maybeDiff: => Option[Diff]) extends Blockchain {
  ...
  override def localDistribution(height: Int): Map[Address, Long] = {
    val innerDistribution = inner.localDistribution(height)
