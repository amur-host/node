...
import com.localplatform.account.{Address, Alias, PublicKeyAccount}
import com.localplatform.transaction.smart.script.Script
import com.localplatform.transaction.{AssetId, Transaction}
...
object Sponsorship {
  ...
  def toLocal(assetFee: Long, sponsorship: Long): Long = {
    val local = (BigDecimal(assetFee) * BigDecimal(Sponsorship.FeeUnit)) / BigDecimal(sponsorship)
    if (local > Long.MaxValue) {
      throw new java.lang.ArithmeticException("Overflow")
    }
    local.toLong
  }

  def fromLocal(localFee: Long, sponsorship: Long): Long = {
    val assetFee = (BigDecimal(localFee) / BigDecimal(Sponsorship.FeeUnit)) * BigDecimal(sponsorship)
