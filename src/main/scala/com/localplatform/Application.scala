...
import com.localplatform.actor.RootActorSystem
import com.localplatform.api.http._
import com.localplatform.api.http.alias.{AliasApiRoute, AliasBroadcastApiRoute}
import com.localplatform.api.http.assets.{AssetsApiRoute, AssetsBroadcastApiRoute}
import com.localplatform.api.http.leasing.{LeaseApiRoute, LeaseBroadcastApiRoute}
import com.localplatform.consensus.PoSSelector
import com.localplatform.consensus.nxt.api.http.NxtConsensusApiRoute
import com.localplatform.db.openDB
import com.localplatform.features.api.ActivationApiRoute
import com.localplatform.history.{CheckpointServiceImpl, StorageFactory}
import com.localplatform.http.{DebugApiRoute, NodeApiRoute, LocalApiRoute}
import com.localplatform.matcher.Matcher
import com.localplatform.metrics.Metrics
import com.localplatform.mining.{Miner, MinerImpl}
import com.localplatform.network.RxExtensionLoader.RxExtensionLoaderShutdownHook
import com.localplatform.network._
import com.localplatform.settings._
import com.localplatform.state.appender.{BlockAppender, CheckpointAppender, ExtensionAppender, MicroblockAppender}
import com.localplatform.transaction._
import com.localplatform.utils.{NTP, ScorexLogging, SystemInformationReporter, Time, forceStopApplication}
import com.localplatform.utx.{MatcherUtxPool, UtxPool, UtxPoolImpl}
...
class Application(val actorSystem: ActorSystem, val settings: LocalSettings, configRoot: ConfigObject) extends ScorexLogging {
  ...
  def run(): Unit = {
    ...
    if (settings.restAPISettings.enable) {
      val apiRoutes = Seq(
        ...
        LocalApiRoute(settings.restAPISettings, wallet, utxStorage, allChannels, time),
...
object Application extends ScorexLogging {
    ...
    val config = maybeConfigFile match {
      ...
      case Some(file) =>
        ...
        if (!cfg.hasPath("local")) {
          ...
          log.error("https://github.com/localplatform/Local/wiki/Local-Node-configuration-file")
  ...
  def main(args: Array[String]): Unit = {
    ...
    System.setProperty("local.directory", config.getString("local.directory"))
    log.info("Starting...")
    sys.addShutdownHook {
    ...
    RootActorSystem.start("localplatform", config) { actorSystem =>
