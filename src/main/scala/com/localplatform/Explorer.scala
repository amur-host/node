...
import com.localplatform.account.{Address, AddressScheme}
import com.localplatform.database.{Keys, LevelDBWriter}
...
import com.localplatform.utils.{Base58, Base64, ScorexLogging}
...
object Explorer extends ScorexLogging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val configFilename = Try(args(0)).toOption.getOrElse("local-testnet.conf")

    val settings = LocalSettings.fromConfig(loadConfig(ConfigFactory.parseFile(new File(configFilename))))
      ...
      flag match {
        ...
        case "A" =>
          ...
          val balances = wbh.map { h =>
            val k = Keys.localBalance(addressId)(h)
