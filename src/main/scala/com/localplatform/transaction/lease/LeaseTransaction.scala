package com.localplatform.transaction.lease

import com.google.common.primitives.{Bytes, Longs}
import monix.eval.Coeval
import play.api.libs.json.{JsObject, Json}
import com.localplatform.account.{Address, AddressOrAlias, PublicKeyAccount}
import com.localplatform.transaction.{AssetId, ProvenTransaction, ValidationError, VersionedTransaction}
...
object LeaseTransaction {
  ...
  def validateLeaseParams(amount: Long, fee: Long, recipient: AddressOrAlias, sender: PublicKeyAccount) =
    if (amount <= 0) {
      Left(ValidationError.NegativeAmount(amount, "local"))
