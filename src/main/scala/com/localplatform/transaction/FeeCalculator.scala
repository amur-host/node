...
import com.localplatform.settings.{FeesSettings, FunctionalitySettings}
import com.localplatform.state._
import com.localplatform.transaction.FeeCalculator._
...
class FeeCalculator(settings: FeesSettings, blockchain: Blockchain) {
  ...
  private val map: Map[String, Long] = {
    settings.fees.flatMap { fs =>
      ...
      fs._2.map { v =>
        val maybeAsset = if (v.asset.toUpperCase == "LOCAL") None else Some(ByteStr.decodeBase58(v.asset).get)
        val fee        = v.fee
  ...
  def enoughFee[T <: Transaction](tx: T): Either[ValidationError, T] = {
    ...
    for {
      ...
      _ <- Either.cond(
        ...
        (),
        GenericError {
          s"Fee in ${txFeeAssetId.fold("LOCAL")(_.toString)} for ${tx.builder.classTag} transaction does not exceed minimal value of $minTxFee"
        }
      )
    } yield tx
  }
