...
object FunctionalitySettings {
  ...
  val configPath = "local.blockchain.custom.functionality"
}
...
object GenesisSettings {
  val MAINNET = GenesisSettings(
    1460678400000L,
    1465742577614L,
    Constants.UnitsInLcl * Constants.TotalLocal,
    ByteStr.decodeBase58("FSH8eAAzZNqnG8xgTZtz5xuLqXySsXgAjmFEC25hXMbEufiGjqWPnGCZFt6gLiVLJny16ipxRNAkkzjjhqTjBE2").toOption,
    List(
      GenesisTransactionSettings("3PAWwWa6GbwcJaFzwqXQN5KQm7H96Y7SHTQ", Constants.UnitsInLcl * Constants.TotalLocal - 5 * Constants.UnitsInLcl),
      GenesisTransactionSettings("3P8JdJGYc7vaLu4UXUZc1iRLdzrkGtdCyJM", Constants.UnitsInLcl),
      GenesisTransactionSettings("3PAGPDPqnGkyhcihyjMHe9v36Y4hkAh9yDy", Constants.UnitsInLcl),
      GenesisTransactionSettings("3P9o3ZYwtHkaU1KxsKkFjJqJKS3dLHLC9oF", Constants.UnitsInLcl),
      GenesisTransactionSettings("3PJaDyprvekvPXPuAtxrapacuDJopgJRaU3", Constants.UnitsInLcl),
      GenesisTransactionSettings("3PBWXDFUc86N2EQxKJmW8eFco65xTyMZx6J", Constants.UnitsInLcl)
    ),
    153722867L,
    60.seconds
  )
  ...
  val TESTNET = GenesisSettings(
    ...
    List(
      GenesisTransactionSettings("3My3KZgFQ3CrVHgz6vGRt8687sH4oAA1qp8", (Constants.UnitsInLcl * Constants.TotalLocal * 0.04).toLong),
      GenesisTransactionSettings("3NBVqYXrapgJP9atQccdBPAgJPwHDKkh6A8", (Constants.UnitsInLcl * Constants.TotalLocal * 0.02).toLong),
      GenesisTransactionSettings("3N5GRqzDBhjVXnCn44baHcz2GoZy5qLxtTh", (Constants.UnitsInLcl * Constants.TotalLocal * 0.02).toLong),
      GenesisTransactionSettings("3NCBMxgdghg4tUhEEffSXy11L6hUi6fcBpd", (Constants.UnitsInLcl * Constants.TotalLocal * 0.02).toLong),
      GenesisTransactionSettings("3N18z4B8kyyQ96PhN5eyhCAbg4j49CgwZJx",
                                 (Constants.UnitsInLcl * Constants.TotalLocal - Constants.UnitsInLcl * Constants.TotalLocal * 0.1).toLong)
    ),
    153722867L,
    60.seconds
  )
...
object BlockchainSettings {
  val configPath: String = "local.blockchain"
  ...
  def fromConfig(config: Config): BlockchainSettings = {
    ...
    val (addressSchemeCharacter, functionalitySettings, genesisSettings) = blockchainType match {
      ...
      case BlockchainType.CUSTOM =>
        ...
        val functionalitySettings  = config.as[FunctionalitySettings]("local.blockchain.custom.functionality")
        val genesisSettings        = config.as[GenesisSettings]("local.blockchain.custom.genesis")
