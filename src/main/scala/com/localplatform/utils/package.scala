...
import com.localplatform.db.{Storage, VersionedStorage}
import com.localplatform.lang.Global
import com.localplatform.lang.v1.compiler.CompilerContext
...
import com.localplatform.lang.v1.{FunctionHeader, ScriptEstimator}
import com.localplatform.transaction.smart.{BlockchainContext, LocalEnvironment}
...
package object utils extends ScorexLogging {
  ...
  def forceStopApplication(reason: ApplicationStopReason = Default): Unit =
    ...
    }, "local-platform-shutdown-thread").start()
  ...
  lazy val dummyCompilerContext: CompilerContext =
    Monoid.combineAll(
      Seq(
        ...
        LocalContext.build(new LocalEnvironment(dummyNetworkByte, Coeval(???), Coeval(???), null)).compilerContext,
