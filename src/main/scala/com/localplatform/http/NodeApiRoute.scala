...
import com.localplatform.Shutdownable
import com.localplatform.settings.{Constants, RestAPISettings}
import com.localplatform.state.Blockchain
import io.swagger.annotations._
import javax.ws.rs.Path
import play.api.libs.json.Json
import com.localplatform.api.http.{ApiRoute, CommonApiFunctions}
...
case class NodeApiRoute(settings: RestAPISettings, blockchain: Blockchain, application: Shutdownable)
  ...
  @Path("/version")
  @ApiOperation(value = "Version", notes = "Get Local node version", httpMethod = "GET")
  @ApiResponses(
    Array(
      new ApiResponse(code = 200, message = "Json Local node version")
    ))
