...
import com.localplatform.api.http.{ApiRoute, DiscontinuedApi}
...
@Api(value = "local")
@Deprecated
case class LocalApiRoute(settings: RestAPISettings, wallet: Wallet, utx: UtxPool, allChannels: ChannelGroup, time: Time)
    extends ApiRoute
    with BroadcastRoute {

  override lazy val route = pathPrefix("local") {
