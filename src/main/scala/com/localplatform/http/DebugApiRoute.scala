...
import com.localplatform.mining.{Miner, MinerDebugInfo}
import com.localplatform.network.{LocalScoreChanged, PeerDatabase, PeerInfo, _}
import com.localplatform.settings.LocalSettings
import com.localplatform.state.{ByteStr, LeaseBalance, NG, Portfolio}
import com.localplatform.transaction._
import com.localplatform.utils.{Base58, ScorexLogging}
...
@Path("/debug")
@Api(value = "/debug")
case class DebugApiRoute(ws: LocalSettings,
                         wallet: Wallet,
                         ng: NG,
  ...
  private lazy val localConfig: JsObject = Json.obj("local" -> (fullConfig \ "local").get)

  override val settings = ws.restAPISettings
  override lazy val route: Route = pathPrefix("debug") {
    blocks ~ state ~ info ~ stateLocal ~ rollback ~ rollbackTo ~ blacklist ~ portfolios ~ minerInfo ~ historyInfo ~ configInfo ~ print
  }
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.localplatform.http.DebugMessage",
        defaultValue = "{\n\t\"message\": \"foo\"\n}"
      )
    ))
  ...
  def state: Route = (path("state") & get & withAuth) {
    complete(ng.localDistribution(ng.height).map { case (a, b) => a.stringRepr -> b })
  }

  @Path("/stateLocal/{height}")
  @ApiOperation(value = "State at block", notes = "Get state at specified height", httpMethod = "GET")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "height", value = "height", required = true, dataType = "integer", paramType = "path")
    ))
  def stateLocal: Route = (path("stateLocal" / IntNumber) & get & withAuth) { height =>
    complete(ng.localDistribution(height).map { case (a, b) => a.stringRepr -> b })
  }
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.localplatform.http.RollbackParams",
  ...
  def configInfo: Route = (path("configInfo") & get & parameter('full.as[Boolean]) & withAuth) { full =>
    complete(if (full) fullConfig else localConfig)
  }
