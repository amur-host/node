...
import com.localplatform.transaction.smart.script.{Script, ScriptReader}
...
object Keys {
  ...
  def localBalanceHistory(addressId: BigInt): Key[Seq[Int]] = historyKey(5, addressId.toByteArray)
  def localBalance(addressId: BigInt)(height: Int): Key[Long] =
  ...
  val addressesForLocalSeqNr: Key[Int]                = intKey(37)
  def addressesForLocal(seqNr: Int): Key[Seq[BigInt]] = Key(h(38, seqNr), readBigIntSeq, writeBigIntSeq)
