package com.localplatform.database

import cats.kernel.Monoid
import com.google.common.cache.CacheBuilder
import com.localplatform.database.patch.DisableHijackedAliases
...
import com.localplatform.state.reader.LeaseDetails
import org.iq80.leveldb.{DB, ReadOptions}
import com.localplatform.account.{Address, Alias}
import com.localplatform.utils.ScorexLogging
import com.localplatform.block.{Block, BlockHeader}
import com.localplatform.transaction.Transaction.Type
import com.localplatform.transaction.ValidationError.{AliasDoesNotExist, AliasIsDisabled}
...
import com.localplatform.transaction.smart.script.Script
...
object LevelDBWriter {
  ...
  /** {{{([15, 12, 3], [12, 5]) => [(15, 12), (12, 12), (3, 12), (3, 5)]}}}
    *
    * @param wbh LOCAL balance history
...
class LevelDBWriter(writableDB: DB, fs: FunctionalitySettings, val maxCacheSize: Int = 100000) extends Caches with ScorexLogging {
  ...
  override def balance(address: Address, mayBeAssetId: Option[AssetId]): Long = readOnly { db =>
    addressId(address).fold(0L) { addressId =>
      mayBeAssetId match {
        ...
        case None          => db.fromHistory(Keys.localBalanceHistory(addressId), Keys.localBalance(addressId)).getOrElse(0L)
      }
    }
  }

  private def loadLposPortfolio(db: ReadOnlyDB, addressId: BigInt) = Portfolio(
    db.fromHistory(Keys.localBalanceHistory(addressId), Keys.localBalance(addressId)).getOrElse(0L),
  ...
  override protected def doAppend(block: Block,
    ...
    val updatedBalanceAddresses = for ((addressId, balance) <- localBalances) yield {
      val kwbh = Keys.localBalanceHistory(addressId)
      val wbh  = rw.get(kwbh)
      if (wbh.isEmpty) {
        newAddressesForLocal += addressId
      }
      rw.put(Keys.localBalance(addressId)(height), balance)
      expiredKeys ++= updateHistory(rw, wbh, kwbh, threshold, Keys.localBalance(addressId))
      addressId
    }

    val changedAddresses = (addressTransactions.keys ++ updatedBalanceAddresses)

    if (newAddressesForLocal.nonEmpty) {
      val newSeqNr = rw.get(Keys.addressesForLocalSeqNr) + 1
      rw.put(Keys.addressesForLocalSeqNr, newSeqNr)
      rw.put(Keys.addressesForLocal(newSeqNr), newAddressesForLocal)
    }
      ...
      val discardedBlocks: Seq[Block] = for (currentHeight <- height until targetHeight by -1) yield {
        ...
        val discardedBlock = readWrite { rw =>
          ...
          for (addressId <- rw.get(Keys.changedAddresses(currentHeight))) {
            ...
            rw.delete(Keys.localBalance(addressId)(currentHeight))
            rw.filterHistory(Keys.localBalanceHistory(addressId), currentHeight)
  ...
  override def balanceSnapshots(address: Address, from: Int, to: Int): Seq[BalanceSnapshot] = readOnly { db =>
    db.get(Keys.addressId(address)).fold(Seq(BalanceSnapshot(1, 0, 0, 0))) { addressId =>
      val wbh = slice(db.get(Keys.localBalanceHistory(addressId)), from, to)
      val lbh = slice(db.get(Keys.leaseBalanceHistory(addressId)), from, to)
      for {
        (wh, lh) <- merge(wbh, lbh)
        wb = balanceAtHeightCache.get((wh, addressId), () => db.get(Keys.localBalance(addressId)(wh)))
  ...
  override def localDistribution(height: Int): Map[Address, Long] = readOnly { db =>
    (for {
      seqNr     <- (1 to db.get(Keys.addressesForLocalSeqNr)).par
      addressId <- db.get(Keys.addressesForLocal(seqNr)).par
      history = db.get(Keys.localBalanceHistory(addressId))
      actualHeight <- history.partition(_ > height)._2.headOption
      balance = db.get(Keys.localBalance(addressId)(actualHeight))
      if balance > 0
    } yield db.get(Keys.idToAddress(addressId)) -> balance).toMap.seq
  }
}
