...
import com.localplatform.matcher.api.{BadMatcherResponse, MatcherResponse}
import com.localplatform.matcher.market.OrderBookActor.{CancelOrder, GetOrderStatusResponse}
import com.localplatform.matcher.market.OrderHistoryActor.{ExpirableOrderHistoryRequest, _}
import com.localplatform.matcher.model.Events.{OrderAdded, OrderCanceled, OrderExecuted}
import com.localplatform.matcher.model.LimitOrder.Filled
...
import com.localplatform.transaction.{AssetAcc, AssetId}
...
class OrderHistoryActor(db: DB, val settings: MatcherSettings, val utxPool: UtxPool, val wallet: Wallet) extends Actor with OrderValidator {
  ...
  def processExpirableRequest(r: Any): Unit = r match {
    ...
    case GetActiveOrdersByAddress(requestId, addr, assets, _) =>
      // Because all orders spend local for fee
...
object OrderHistoryActor {
  ...
  case class DeleteOrderFromHistory(assetPair: AssetPair, req: com.localplatform.matcher.api.CancelOrderRequest)
