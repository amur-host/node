...
import com.localplatform.matcher.market.BalanceWatcherWorkerActor._
import com.localplatform.matcher.market.OrderBookActor.ForceCancelOrder
import com.localplatform.matcher.market.OrderHistoryActor.{ForceCancelOrderFromHistory, GetActiveOrdersByAddress, GetActiveOrdersByAddressResponse}
import com.localplatform.matcher.model.Events.BalanceChanged
import com.localplatform.matcher.model.LimitOrder
import com.localplatform.state.LeaseBalance
        ...
        val (_, r) = ordersByPriority.foldLeft((portfolio, List.empty: OrdersToDelete)) {
          case ((restPortfolio, toDelete), limitOrder) =>
            val updatedPortfolio = restPortfolio
              ...
              .flatMap { p =>
                if (limitOrder.rcvAsset == AssetPair.LocalName && limitOrder.getReceiveAmount >= limitOrder.remainingFee) Some(p)
