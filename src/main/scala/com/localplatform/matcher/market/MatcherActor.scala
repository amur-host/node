...
import com.localplatform.matcher.api.{MatcherResponse, StatusCodeMatcherResponse}
...
import com.localplatform.utils.{Base58, NTP, ScorexLogging}
...
class MatcherActor(orderHistory: ActorRef,
  ...
  def getAssetName(asset: Option[AssetId]): String =
    asset.fold(AssetPair.LocalName) { aid =>
