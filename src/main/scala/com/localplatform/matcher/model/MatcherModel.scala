...
import com.localplatform.state.Portfolio
...
sealed trait LimitOrder {
  ...
  def spentAsset: String = order.getSpendAssetId.map(_.base58).getOrElse(AssetPair.LocalName)
  def rcvAsset: String   = order.getReceiveAssetId.map(_.base58).getOrElse(AssetPair.LocalName)
  def feeAsset: String   = AssetPair.LocalName
}
