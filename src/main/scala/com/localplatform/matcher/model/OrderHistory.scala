...
import com.localplatform.db.{AssetIdOrderIdSetCodec, Codec, OrderIdsCodec, PortfolioCodec, SubStorage}
import com.localplatform.matcher.MatcherSettings
import com.localplatform.matcher.model.Events.{Event, OrderAdded, OrderCanceled, OrderExecuted}
import com.localplatform.matcher.model.LimitOrder.{Filled, OrderStatus}
import com.localplatform.matcher.model.OrderHistory.OrderHistoryOrdering
...
case class OrderHistoryImpl(db: DB, settings: MatcherSettings) extends SubStorage(db: DB, "matcher") with OrderHistory with ScorexLogging {
  ...
  override def order(id: String): Option[Order] = {
    import com.localplatform.transaction.assets.exchange.OrderJson.orderFormat
  ...
  override def openVolume(assetAcc: AssetAcc): Long = {
    val asset = assetAcc.assetId.map(_.base58).getOrElse(AssetPair.LocalName)
