...
import com.amurplatform.state.diffs.{ENOUGH_AMT, assertDiffAndState, produce}
...
import com.amurplatform.account.{AddressOrAlias, AddressScheme, PrivateKeyAccount}
import com.amurplatform.lagonaki.mocks.TestBlock
import com.amurplatform.transaction.smart.BlockchainContext
import com.amurplatform.transaction.transfer._
import com.amurplatform.transaction.{CreateAliasTransaction, GenesisTransaction, Transaction}
import shapeless._
...
class AddressFromRecipientScenarioTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink {
  ...
  def evalScript(tx: Transaction, blockchain: Blockchain): Either[com.amurplatform.lang.ExecutionError, CaseObj] = {
    val context =
    ...
    val Parsed.Success(expr, _) = Parser("""
        ...
        |  """.stripMargin)
    assert(expr.size == 1)
    val Right((typedExpr, _)) = CompilerV1(com.amurplatform.utils.dummyCompilerContext, expr.head)
