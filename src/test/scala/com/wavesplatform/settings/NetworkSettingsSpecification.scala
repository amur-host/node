...
class NetworkSettingsSpecification extends FlatSpec with Matchers {
  ...
  "NetworkSpecification" should "read values from config" in {
    val config          = loadConfig(ConfigFactory.parseString("""amur.network {
  ...
  it should "build node name using nonce" in {
    val config          = loadConfig(ConfigFactory.parseString("amur.network.nonce = 12345"))
    val networkSettings = config.as[NetworkSettings]("amur.network")
  ...
  it should "fail with IllegalArgumentException on too long node name" in {
    val config = loadConfig(ConfigFactory.parseString(
      "amur.network.node-name = очень-длинное-название-в-многобайтной-кодировке-отличной-от-однобайтной-кодировки-американского-института-стандартов"))
    intercept[IllegalArgumentException] {
      config.as[NetworkSettings]("amur.network")
    }
  }
}
