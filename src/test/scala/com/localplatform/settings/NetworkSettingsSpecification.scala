...
class NetworkSettingsSpecification extends FlatSpec with Matchers {
  ...
  "NetworkSpecification" should "read values from config" in {
    val config          = loadConfig(ConfigFactory.parseString("""local.network {
  ...
  it should "build node name using nonce" in {
    val config          = loadConfig(ConfigFactory.parseString("local.network.nonce = 12345"))
    val networkSettings = config.as[NetworkSettings]("local.network")
  ...
  it should "fail with IllegalArgumentException on too long node name" in {
    val config = loadConfig(ConfigFactory.parseString(
      "local.network.node-name = очень-длинное-название-в-многобайтной-кодировке-отличной-от-однобайтной-кодировки-американского-института-стандартов"))
    intercept[IllegalArgumentException] {
      config.as[NetworkSettings]("local.network")
    }
  }
}
