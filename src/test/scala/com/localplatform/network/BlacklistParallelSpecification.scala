...
import com.localplatform.settings.{NetworkSettings, loadConfig}
...
class BlacklistParallelSpecification extends FeatureSpec with GivenWhenThen with ParallelTestExecution {
  ...
  private val config = loadConfig(ConfigFactory.parseString("""local.network {
      |  known-peers = []
      |  file = null
      |  black-list-residence-time: 1s
      |}""".stripMargin))

  private val networkSettings = config.as[NetworkSettings]("local.network")
