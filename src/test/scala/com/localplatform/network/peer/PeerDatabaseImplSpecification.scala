package com.localplatform.network.peer
...
import com.localplatform.network.PeerDatabaseImpl
import com.localplatform.settings.NetworkSettings
import net.ceedubs.ficus.Ficus._
import org.scalatest.{Matchers, path}

class PeerDatabaseImplSpecification extends path.FreeSpecLike with Matchers {

  private val config1   = ConfigFactory.parseString("""local.network {
      |  file = null
      |  known-peers = []
      |  peers-data-residence-time: 2s
      |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
  private val settings1 = config1.as[NetworkSettings]("local.network")

  private val config2   = ConfigFactory.parseString("""local.network {
      |  file = null
      |  known-peers = []
      |  peers-data-residence-time: 10s
      |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
  private val settings2 = config2.as[NetworkSettings]("local.network")
  ...
  "Peer database" - {
    ...
    "if blacklisting is disabled" - {
      "should clear blacklist at start" in {
        val databaseFile = Files.createTempFile("local-tests", "PeerDatabaseImplSpecification-blacklisting-clear").toAbsolutePath.toString
        val path         = if (File.separatorChar == '\\') databaseFile.replace('\\', '/') else databaseFile
        val prevConfig   = ConfigFactory.parseString(s"""local.network {
             |  file = "$path"
             |  known-peers = []
             |  peers-data-residence-time: 100s
             |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
        val prevSettings = prevConfig.as[NetworkSettings]("local.network")
        val prevDatabase = new PeerDatabaseImpl(prevSettings)
        prevDatabase.blacklist(address1, "I don't like it")
        prevDatabase.close()

        val config   = ConfigFactory.parseString(s"""local.network {
             |  file = "$path"
             |  known-peers = []
             |  peers-data-residence-time: 100s
             |  enable-blacklisting = no
             |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
        val settings = config.as[NetworkSettings]("local.network")
