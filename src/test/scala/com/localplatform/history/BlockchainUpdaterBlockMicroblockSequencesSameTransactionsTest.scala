...
class BlockchainUpdaterBlockMicroblockSequencesSameTransactionsTest
  ...
  property("Miner fee from microblock [Genesis] <- [Empty] <~ (Micro with tx) <- [Empty]") {
    val preconditionsAndPayments: Gen[(PrivateKeyAccount, GenesisTransaction, TransferTransactionV1, Int)] = for {
      ...
      payment: TransferTransactionV1 = createLocalTransfer(master, master, amt, fee, ts).explicitGet()
  ...
  def randomPayment(accs: Seq[PrivateKeyAccount], ts: Long): Gen[TransferTransactionV1] =
    for {
      ...
      fee  <- smallFeeGen
      amt  <- smallFeeGen
    } yield createLocalTransfer(from, to, amt, fee, ts).explicitGet()
  ...
  def accsAndGenesis(): Gen[(Seq[PrivateKeyAccount], PrivateKeyAccount, Block, Int)] =
    for {
      ...
      miner   <- accountGen
      ts      <- positiveIntGen
      genesis1: GenesisTransaction = GenesisTransaction.create(alice, TOTAL_LOCAL / 4, ts).explicitGet()
      genesis2: GenesisTransaction = GenesisTransaction.create(bob, TOTAL_LOCAL / 4, ts + 1).explicitGet()
      genesis3: GenesisTransaction = GenesisTransaction.create(charlie, TOTAL_LOCAL / 4, ts + 2).explicitGet()
      genesis4: GenesisTransaction = GenesisTransaction.create(dave, TOTAL_LOCAL / 4, ts + 4).explicitGet()
    } yield
