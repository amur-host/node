...
import com.localplatform.account.{Address, AddressOrAlias, PrivateKeyAccount}
...
class BlockchainUpdaterMicroblockSunnyDayTest
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    masterToAlice: TransferTransactionV1 <- localTransferGeneratorP(master, alice)
    aliceToBob  = createLocalTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts).explicitGet()
    aliceToBob2 = createLocalTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts + 1).explicitGet()
  ...
  property("discarding some of microBlocks doesn't affect resulting state") {
    forAll(preconditionsAndPayments, accountGen) {
      case ((genesis, masterToAlice, aliceToBob, aliceToBob2), miner) =>
        ...
        val minerABalance = withDomain(MicroblocksActivatedAt0LocalSettings) { da =>
        ...
        val minerBBalance = withDomain(MicroblocksActivatedAt0LocalSettings) { db =>
