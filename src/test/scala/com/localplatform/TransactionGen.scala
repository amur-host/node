...
import com.localplatform.lang.v1.testing.ScriptGen
import com.localplatform.settings.Constants
import com.localplatform.state._
import com.localplatform.state.diffs.ENOUGH_AMT
import org.scalacheck.Gen.{alphaLowerChar, alphaUpperChar, frequency, numChar}
import org.scalacheck.{Arbitrary, Gen}
import org.scalatest.{BeforeAndAfterAll, Suite}
import com.localplatform.account.PublicKeyAccount._
import com.localplatform.account._
...
import com.localplatform.transaction.lease._
...
import com.localplatform.transaction.transfer.MassTransferTransaction.ParsedTransfer
import com.localplatform.transaction.transfer._
import MassTransferTransaction.MaxTransferCount
import com.localplatform.utils.TimeImpl
...
trait TransactionGenBase extends ScriptGen {
  ...
  protected def local(n: Float): Long = (n * 100000000L).toLong
  ...
  val localAssetGen: Gen[Option[ByteStr]] = Gen.const(None)
  val assetIdGen: Gen[Option[ByteStr]]    = Gen.frequency((1, localAssetGen), (10, Gen.option(bytes32gen.map(ByteStr(_)))))
  ...
  def localTransferGeneratorP(sender: PrivateKeyAccount, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
    transferGeneratorP(sender, recipient, None, None)

  def localTransferGeneratorP(timestamp: Long, sender: PrivateKeyAccount, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
  ...
  def createLocalTransfer(sender: PrivateKeyAccount,
                          recipient: Address,
                          amount: Long,
                          fee: Long,
  ...
  val transferWithLocalFeeGen = for {
  ...
  val selfTransferWithLocalFeeGen: Gen[TransferTransactionV1] = for {
  ...
  def sponsorFeeCancelSponsorFeeGen(
    ...
    for {
      ...
      (issue,
       SponsorFeeTransaction.selfSigned(1, sender, assetId, Some(minFee), 1 * Constants.UnitsInLcl, timestamp).explicitGet(),
       SponsorFeeTransaction.selfSigned(1, sender, assetId, Some(minFee1), 1 * Constants.UnitsInLcl, timestamp).explicitGet(),
       SponsorFeeTransaction.selfSigned(1, sender, assetId, None, 1 * Constants.UnitsInLcl, timestamp).explicitGet(),
      )
