package com.localplatform.lagonaki.unit
...
import com.localplatform.{NoShrink, TransactionGen, crypto}
...
class BlockSpecification extends PropSpec with PropertyChecks with TransactionGen with Matchers with NoShrink {
  ...
  val blockGen = for {
    ...
    paymentTransaction        <- localTransferGeneratorP(time, sender, recipient)
    transferTrancation        <- transferGeneratorP(1 + time, sender, recipient, assetId, None)
    anotherPaymentTransaction <- localTransferGeneratorP(2 + time, sender, recipient)
  ...
  def bigBlockGen(amt: Int): Gen[Block] =
    for {
      ...
      sender                                    <- accountGen
      recipient                                 <- accountGen
      paymentTransaction: TransferTransactionV1 <- localTransferGeneratorP(time, sender, recipient)
    } yield
      Block
        .buildAndSign(3,
                      time,
