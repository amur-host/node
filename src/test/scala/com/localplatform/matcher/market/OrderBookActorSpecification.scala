...
import com.localplatform.matcher.model.Events.Event
import com.localplatform.matcher.model.{BuyLimitOrder, LimitOrder, SellLimitOrder}
import com.localplatform.settings.{Constants, FunctionalitySettings, TestFunctionalitySettings, WalletSettings}
...
import com.localplatform.transaction.assets.exchange.{AssetPair, ExchangeTransaction, Order}
...
class OrderBookActorSpecification
    ...
    .returns(
      Portfolio(hugeAmount,
                ...
                Map(
                  ...
                  ByteStr("LOCAL".getBytes) -> hugeAmount
                )))
  ...
  "OrderBookActror" should {
    ...
    "partially execute order with zero fee remaining part" in {
      ...
      val ord1 = sell(pair, 0.0006999, 1500 * Constants.UnitsInLcl)
      ...
      expectMsg(
        GetOrdersResponse(
          Seq(SellLimitOrder((0.0006999 * Order.PriceConstant).toLong, 1500 * Constants.UnitsInLcl - (3075363900L - 3075248828L), ord1))))
    }
    ...
    "partially execute order with price > 1 and zero fee remaining part " in {
      ...
      val ord1 = sell(pair, 1850, (0.1 * Constants.UnitsInLcl).toLong)
      val ord2 = sell(pair, 1840, (0.01 * Constants.UnitsInLcl).toLong)
      val ord3 = buy(pair, 2000, (0.0100001 * Constants.UnitsInLcl).toLong)
      ...
      actor ! GetAskOrdersRequest
      expectMsg(GetOrdersResponse(Seq(SellLimitOrder(1850 * Order.PriceConstant, ((0.1 - (0.0100001 - 0.01)) * Constants.UnitsInLcl).toLong, ord1))))
    }
