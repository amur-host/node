...
import com.localplatform.matcher.model.{BuyLimitOrder, SellLimitOrder}
...
trait MatcherTestData {
  ...
  val localAssetGen: Gen[Option[Array[Byte]]] = Gen.const(None)
  val assetIdGen: Gen[Option[Array[Byte]]]    = Gen.frequency((1, localAssetGen), (10, bytes32gen.map(Some(_))))
  ...
  val maxLocalAmountGen: Gen[Long] = Gen.choose(1, 100000000L * 100000000L)
  ...
  def buyGenerator(pair: AssetPair,
    ...
    for {
      ...
      matcherFee: Long          <- matcherFee.map(Gen.const).getOrElse(maxLocalAmountGen)
  ...
  val orderGenerator: Gen[(Order, PrivateKeyAccount)] = for {
    ...
    price: Long               <- maxLocalAmountGen
    amount: Long              <- maxLocalAmountGen
    timestamp: Long           <- createdTimeGen
    expiration: Long          <- maxTimeGen
    matcherFee: Long          <- maxLocalAmountGen
