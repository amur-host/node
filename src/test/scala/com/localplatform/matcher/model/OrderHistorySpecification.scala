...
class OrderHistorySpecification
  ...
  property("New buy LOCAL order added") {
  ...
  property("New sell LOCAL order added") {
    val pair = AssetPair(None, Some(ByteStr("BTC".getBytes)))
  ...
  property("New buy and sell LOCAL order added") {
  ...
  property("Buy LOCAL order filled") {
    val pair = AssetPair(None, Some(ByteStr("BTC".getBytes)))
    val ord1 = buy(pair, 0.0008, 10000)
  ...
  property("Sell LOCAL order - filled, buy order - partial") {
    val pair = AssetPair(None, Some(ByteStr("BTC".getBytes)))
  ...
  property("Buy LOCAL order - filled with 2 steps, sell order - partial") {
    val pair = AssetPair(None, Some(ByteStr("BTC".getBytes)))
  ...
  property("Open Portfolio for two assets") {
    ...
    oh.openPortfolio(pk.address) shouldBe
      OpenPortfolio(
        Map("LOCAL"     -> (2 * matcherFee - LimitOrder(ord1).getReceiveAmount - LimitOrder(ord2).getReceiveAmount),
