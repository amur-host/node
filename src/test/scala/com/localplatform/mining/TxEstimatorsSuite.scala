...
import com.localplatform.state.{AssetDescription, Blockchain, ByteStr, EitherExt2}
...
class TxEstimatorsSuite extends FreeSpec with Matchers with PathMockFactory with TransactionGen {
  "scriptRunNumber" - {
    "smart account" - {
      "should not count transactions going from a regular account" in {
        ...
        TxEstimators.scriptRunNumber(blockchain, transferLocalTx) shouldBe 0
      }
      ...
      "should count transactions going from a smart account" in {
        ...
        TxEstimators.scriptRunNumber(blockchain, transferLocalTx) shouldBe 1
      }
    }
  ...
  private val transferLocalTx = TransferTransactionV1
    .selfSigned(
      assetId = None,
