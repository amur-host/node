...
import com.localplatform.history._
import com.localplatform.state.diffs._
import com.localplatform.state.{Diff, EitherExt2, NgState}
...
class NgStateTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink {
  ...
  def preconditionsAndPayments(amt: Int): Gen[(GenesisTransaction, Seq[TransferTransactionV1])] =
    for {
      ...
      payments: Seq[TransferTransactionV1] <- Gen.listOfN(amt, localTransferGeneratorP(master, recipient))
