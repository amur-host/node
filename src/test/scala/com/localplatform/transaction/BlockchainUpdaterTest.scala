...
import com.localplatform.features.BlockchainFeatureStatus
import com.localplatform.features.FeatureProvider._
import com.localplatform.history
...
class BlockchainUpdaterTest extends FreeSpec with Matchers with HistoryTest with ShouldVerb with WithState {
  ...
  private val LocalSettings = history.DefaultLocalSettings.copy(
    blockchainSettings = history.DefaultLocalSettings.blockchainSettings.copy(
      functionalitySettings = history.DefaultLocalSettings.blockchainSettings.functionalitySettings.copy(
        featureCheckBlocksPeriod = ApprovalPeriod,
        blocksForFeatureActivation = (ApprovalPeriod * 0.9).toInt,
        preActivatedFeatures = Map.empty
      )
    ),
    featuresSettings = history.DefaultLocalSettings.featuresSettings.copy(autoShutdownOnUnsupportedFeature = true)
  )

  private val LocalSettingsWithDoubling = LocalSettings.copy(
    blockchainSettings = LocalSettings.blockchainSettings.copy(
      functionalitySettings = LocalSettings.blockchainSettings.functionalitySettings.copy(
  ...
  "features approved and accepted as height grows" in withDomain(LocalSettings) { domain =>
  ...
  "features rollback with block rollback" in withDomain(LocalSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "feature activation height is not overriden with further periods" in withDomain(LocalSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "feature activated only by 90% of blocks" in withDomain(LocalSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "features votes resets when voting window changes" in withDomain(LocalSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "sunny day test when known feature activated" in withDomain(LocalSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "empty blocks should not disable activation" in withDomain(LocalSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods works in the middle of activation period" in withDomain(LocalSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods should work after defined height" in withDomain(LocalSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
