package com.localplatform.utx

import com.typesafe.config.ConfigFactory
import com.localplatform.account.{Address, PrivateKeyAccount, PublicKeyAccount}
...
import com.localplatform.lang.v1.compiler.Terms.EXPR
import com.localplatform.lang.v1.compiler.{CompilerContext, CompilerV1}
import com.localplatform.mining._
import com.localplatform.settings._
import com.localplatform.state.diffs._
import com.localplatform.state.{ByteStr, EitherExt2, _}
import com.localplatform.transaction.ValidationError.SenderIsBlacklisted
...
import com.localplatform.transaction.{FeeCalculator, GenesisTransaction, Transaction}
import com.localplatform.utils.Time
import com.localplatform._
...
class UtxPoolSpecification extends FreeSpec with Matchers with MockFactory with PropertyChecks with TransactionGen with NoShrink with WithDB {
  ...
  private val calculatorSettings = FeesSettings(
    ...
    ).map(_.typeId.toInt -> List(FeeSettings("LOCAL", 0))).toMap
  )
  ...
  private def mkBlockchain(senderAccount: Address, senderBalance: Long) = {
    ...
    val origSettings    = LocalSettings.fromConfig(config)
  ...
  private val twoOutOfManyValidPayments = (for {
    ...
    val txs        = for (_ <- 1 to n) yield createLocalTransfer(sender, recipient, amountPart, fee, time.getTimestamp()).explicitGet()
  ...
  private def transactionGen(sender: PrivateKeyAccount, ts: Long, feeAmount: Long): Gen[TransferTransactionV1] = accountGen.map { recipient =>
    TransferTransactionV1.selfSigned(None, sender, recipient, local(1), ts, None, feeAmount, Array.emptyByteArray).explicitGet()
  }
