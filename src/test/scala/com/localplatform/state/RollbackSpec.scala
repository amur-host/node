...
import com.localplatform.crypto.SignatureLength
import com.localplatform.db.WithState
import com.localplatform.state.reader.LeaseDetails
import com.localplatform.{NoShrink, TestTime, TransactionGen}
import org.scalacheck.Gen
import org.scalatest.prop.PropertyChecks
import org.scalatest.{FreeSpec, Matchers}
import com.localplatform.account.{Address, PrivateKeyAccount}
import com.localplatform.lagonaki.mocks.TestBlock
import com.localplatform.transaction.ValidationError.AliasDoesNotExist
import com.localplatform.transaction.assets.{IssueTransactionV1, ReissueTransactionV1}
import com.localplatform.transaction.lease.{LeaseCancelTransactionV1, LeaseTransactionV1}
import com.localplatform.transaction.smart.SetScriptTransaction
import com.localplatform.transaction.transfer._
import com.localplatform.transaction.{CreateAliasTransactionV1, DataTransaction, GenesisTransaction}
import com.localplatform.features._
  ...
  "Rollback resets" - {
    ...
    "forget rollbacked transaction for quering" in forAll(accountGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        ...
        val localSettings = history.DefaultLocalSettings.copy(
          blockchainSettings = history.DefaultLocalSettings.blockchainSettings.copy(functionalitySettings = settings))
        withDomain(localSettings) { d =>
          d.appendBlock(genesisBlock(nextTs, sender, com.localplatform.state.diffs.ENOUGH_AMT))
    ...
    "local balances" in forAll(accountGen, positiveLongGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
