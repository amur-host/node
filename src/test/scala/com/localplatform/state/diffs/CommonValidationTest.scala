...
import com.localplatform.features.{BlockchainFeature, BlockchainFeatures}
import com.localplatform.lang.v1.compiler.Terms._
import com.localplatform.mining.MiningConstraint
import com.localplatform.settings.{Constants, FunctionalitySettings, TestFunctionalitySettings}
...
import com.localplatform.transaction.assets.{IssueTransactionV1, IssueTransactionV2, SponsorFeeTransaction}
...
import com.localplatform.transaction.{GenesisTransaction, Transaction, ValidationError}
...
class CommonValidationTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with WithState with NoShrink {
  ...
  property("disallows double spending") {
    val preconditionsAndPayment: Gen[(GenesisTransaction, TransferTransactionV1)] = for {
      ...
      transfer: TransferTransactionV1 <- localTransferGeneratorP(master, recipient)
  ...
  property("checkFee for smart tokens fails if the fee is in tokens") {
    smartTokensCheckFeeTest(feeInAssets = true, feeAmount = 1)(_ should produce("Transactions with smart tokens require Local as fee"))
  }
  ...
  property("checkFee for smart accounts fails if the fee is in tokens") {
    smartAccountCheckFeeTest(feeInAssets = true, feeAmount = 1)(_ should produce("Transactions from scripted accounts require Local as fee"))
  }
  ...
  private def sponsorAndSetScriptGen(sponsorship: Boolean, smartToken: Boolean, smartAccount: Boolean, feeInAssets: Boolean, feeAmount: Long) =
    for {
        ...
        if (smartToken)
          IssueTransactionV2
            .selfSigned(
              ...
              reissuable = false,
              Some(script),
              Constants.UnitsInLcl,
              ts
            )
            .explicitGet()
        else
          IssueTransactionV1
            .selfSigned(richAcc, "test".getBytes(), "desc".getBytes(), Long.MaxValue, 2, reissuable = false, Constants.UnitsInLcl, ts)
            .explicitGet()

      val transferLocalTx = TransferTransactionV1
        .selfSigned(None, richAcc, recipientAcc, 10 * Constants.UnitsInLcl, ts, None, 1 * Constants.UnitsInLcl, Array.emptyByteArray)
        .explicitGet()

      val transferAssetTx = TransferTransactionV1
        .selfSigned(Some(issueTx.id()), richAcc, recipientAcc, 100, ts, None, 1 * Constants.UnitsInLcl, Array.emptyByteArray)
        .explicitGet()
        ...
        if (sponsorship)
          Seq(
            SponsorFeeTransaction
              .selfSigned(1, richAcc, issueTx.id(), Some(10), Constants.UnitsInLcl, ts)
              .explicitGet()
          )
        else Seq.empty
      ...
      (TestBlock.create(Vector[Transaction](genesisTx, issueTx, transferLocalTx, transferAssetTx) ++ sponsorTx ++ setScriptTx), transferBackTx)
    }
