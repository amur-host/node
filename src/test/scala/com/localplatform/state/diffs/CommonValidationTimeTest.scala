...
class CommonValidationTimeTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithState {
  ...
  property("disallows too old transacions") {
    forAll(for {
      ...
      fee         <- smallFeeGen
      transfer1 = createLocalTransfer(master, recipient, amount, fee, prevBlockTs - CommonValidation.MaxTimePrevBlockOverTransactionDiff.toMillis - 1)
        .explicitGet()
  ...
  property("disallows transactions from far future") {
    forAll(for {
      ...
      amount      <- positiveLongGen
      fee         <- smallFeeGen
      transfer1 = createLocalTransfer(master, recipient, amount, fee, blockTs + CommonValidation.MaxTimeTransactionOverBlockDiff.toMillis + 1)
        .explicitGet()
    } yield (prevBlockTs, blockTs, height, transfer1)) {
      case (prevBlockTs, blockTs, height, transfer1) =>
