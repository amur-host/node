package com.localplatform.state.diffs.smart
...
import com.localplatform.transaction.{DataTransaction, Transaction}
import com.localplatform.transaction.smart.BlockchainContext
import com.localplatform.transaction.transfer.TransferTransaction
...
package object predef {
  ...
  def scriptWithAllFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s"""
       ...
       | }
       |
       | let balances = assetBalance(tx.sender, unit) > 0 && localBalance(tx.sender) != 0
       |
