...
import com.localplatform.lang.v1.evaluator.ctx.CaseObj
import com.localplatform.lang.v1.parser.Parser
import com.localplatform.state._
import com.localplatform.state.diffs.{ENOUGH_AMT, assertDiffAndState, produce}
...
import com.localplatform.account.{AddressOrAlias, AddressScheme, PrivateKeyAccount}
...
import com.localplatform.transaction.{CreateAliasTransaction, GenesisTransaction, Transaction}
import shapeless._
...
class AddressFromRecipientScenarioTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink {
  ...
  def evalScript(tx: Transaction, blockchain: Blockchain): Either[com.localplatform.lang.ExecutionError, CaseObj] = {
    val context =
    ...
    val Parsed.Success(expr, _) = Parser("""
        ...
        |  """.stripMargin)
    assert(expr.size == 1)
    val Right((typedExpr, _)) = CompilerV1(com.localplatform.utils.dummyCompilerContext, expr.head)
