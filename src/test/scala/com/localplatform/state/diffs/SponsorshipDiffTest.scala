...
import com.localplatform.lagonaki.mocks.TestBlock.{create => block}
import com.localplatform.transaction.GenesisTransaction
import com.localplatform.transaction.assets.{IssueTransactionV1, SponsorFeeTransaction}
...
class SponsorshipDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen {
  ...
  property("not enough fee") {
    ...
    val setup = for {
      ...
      localOverspend = TransferTransactionV1
        .selfSigned(None, master, recipient.toAddress, 1000000, ts + 3, Some(assetId), fee, Array.emptyByteArray)
        .right
        .get
    } yield (genesis, issueTx, sponsorTx, assetOverspend, insufficientFee, localOverspend)

    forAll(setup) {
      case (genesis, issue, sponsor, assetOverspend, insufficientFee, localOverspend) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          blockDiffEi should produce("does not exceed minimal value of 100000 LOCAL")
        }
        assertDiffEi(setupBlocks, block(Seq(localOverspend)), s) { blockDiffEi =>
          if (localOverspend.fee > issue.quantity)
            blockDiffEi should produce("unavailable funds")
          else
            blockDiffEi should produce("negative local balance")
        }
    }
  }

  property("not enough local to pay fee after leasing") {
    val s = settings(0)
    val setup = for {
      master <- accountGen
      alice  <- accountGen
  ...
  property("cannot cancel sponsorship") {
    ...
    val setup = for {
      ...
      senderNotIssuer = SponsorFeeTransaction
        .selfSigned(1, notSponsor, assetId, None, 1 * Constants.UnitsInLcl, ts + 1)
        .right
        .get
      insufficientFee = SponsorFeeTransaction
        .selfSigned(1, notSponsor, assetId, None, 1 * Constants.UnitsInLcl - 1, ts + 1)
        .right
        .get
    ...
    forAll(setup) {
      case (genesis, issueTx, sponsorTx, senderNotIssuer, insufficientFee) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) { blockDiffEi =>
          blockDiffEi should produce("does not exceed minimal value of 100000000 LOCAL: 99999999")
        }
    }
  }
  ...
  property("cannot сhange sponsorship fee") {
    ...
    val setup = for {
      ...
      minFee <- smallFeeGen
      senderNotIssuer = SponsorFeeTransaction
        .selfSigned(1, notSponsor, assetId, Some(minFee), 1 * Constants.UnitsInLcl, ts + 1)
        .right
        .get
      insufficientFee = SponsorFeeTransaction
        .selfSigned(1, notSponsor, assetId, Some(minFee), 1 * Constants.UnitsInLcl - 1, ts + 1)
        .right
        .get
    } yield (genesis, issueTx, sponsorTx, senderNotIssuer, insufficientFee)
  ...
  property("sponsor has no LOCAL but receives them just in time") {
    val s = settings(0)
    val setup = for {
      master    <- accountGen
      recipient <- accountGen
      ...
      assetTransfer = TransferTransactionV1
        ...
        .right
        .get
      localTransfer = TransferTransactionV1
        .selfSigned(None, master, recipient, 99800000, ts + 4, None, 100000, Array.emptyByteArray)
        .right
        .get
      backLocalTransfer = TransferTransactionV1
        .selfSigned(None, recipient, master, 100000, ts + 5, Some(assetId), 100, Array.emptyByteArray)
        .right
        .get
    } yield (genesis, issue, sponsor, assetTransfer, localTransfer, backLocalTransfer)

    forAll(setup) {
      case (genesis, issue, sponsor, assetTransfer, localTransfer, backLocalTransfer) =>
        assertDiffAndState(Seq(block(Seq(genesis, issue, sponsor, assetTransfer, localTransfer))), block(Seq(backLocalTransfer)), s) {
          case (diff, state) =>
