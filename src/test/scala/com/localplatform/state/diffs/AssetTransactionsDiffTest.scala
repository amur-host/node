...
class AssetTransactionsDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithDB {
  ...
  property("Issue+Reissue+Burn do not break local invariant and updates state") {
  ...
  property("Can burn non-owned alias if feature 'BurnAnyTokens' activated") {
    val setup = for {
      ...
      localTransfer <- localTransferGeneratorP(issuer, burner)
      burn = BurnTransactionV1.selfSigned(burner, issue.assetId(), assetTransfer.amount, localTransfer.amount, timestamp).explicitGet()
    } yield (genesis, issue, assetTransfer, localTransfer, burn)
    ...
    forAll(setup) {
      case (genesis, issue, assetTransfer, localTransfer, burn) =>
        assertDiffAndState(Seq(TestBlock.create(Seq(genesis, issue, assetTransfer, localTransfer))), TestBlock.create(Seq(burn)), fs) {
          case (_, newState) =>
  ...
  def genesisIssueTransferReissue(code: String): Gen[(Seq[GenesisTransaction], IssueTransactionV2, TransferTransactionV1, ReissueTransactionV1)] =
    for {
      ...
      timestamp          <- timestampGen
      initialLocalAmount <- Gen.choose(Long.MaxValue / 1000, Long.MaxValue / 100)
      accountA           <- accountGen
      accountB           <- accountGen
      smallFee           <- Gen.choose(1l, 10l)
      genesisTx1 = GenesisTransaction.create(accountA, initialLocalAmount, timestamp).explicitGet()
      genesisTx2 = GenesisTransaction.create(accountB, initialLocalAmount, timestamp).explicitGet()
      reissuable = true
