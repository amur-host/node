...
import com.localplatform.settings.{TestFunctionalitySettings, LocalSettings, loadConfig}
import com.localplatform.state.diffs.ENOUGH_AMT
import com.localplatform.state.{BlockchainUpdaterImpl, EitherExt2}
...
import com.localplatform.transaction.transfer.{TransferTransaction, TransferTransactionV1}
import com.localplatform.transaction.{GenesisTransaction, Transaction}
import com.localplatform.utils.{Time, TimeImpl}
import com.localplatform.{RequestGen, WithDB}
...
class LevelDBWriterSpec extends FreeSpec with Matchers with WithDB with RequestGen {
  ...
  def baseTest(gen: Time => Gen[(PrivateKeyAccount, Seq[Block])])(f: (LevelDBWriter, PrivateKeyAccount) => Unit): Unit = {
    ...
    val settings0     = LocalSettings.fromConfig(loadConfig(ConfigFactory.load()))
