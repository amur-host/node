...
class TxEstimatorsSuite extends FreeSpec with Matchers with PathMockFactory with TransactionGen {
  "scriptRunNumber" - {
    "smart account" - {
      "should not count transactions going from a regular account" in {
        ...
        TxEstimators.scriptRunNumber(blockchain, transferAmurTx) shouldBe 0
      }
      ...
      "should count transactions going from a smart account" in {
        ...
        TxEstimators.scriptRunNumber(blockchain, transferAmurTx) shouldBe 1
      }
    }
  ...
  private val transferAmurTx = TransferTransactionV1
    .selfSigned(
      assetId = None,
