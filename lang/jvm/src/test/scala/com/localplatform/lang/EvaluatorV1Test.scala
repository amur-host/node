...
import com.localplatform.lang.v1.evaluator.ctx.impl.{CryptoContext, EnvironmentFunctions, PureContext}
...
import com.localplatform.utils.{Base58, Base64}
...
class EvaluatorV1Test extends PropSpec with PropertyChecks with Matchers with ScriptGen with NoShrink {
  ...
  private def defaultFullContext(environment: Environment): CTX = Monoid.combineAll(
    Seq(
      ...
      PureContext.ctx,
      LocalContext.build(environment)
    )
  )
